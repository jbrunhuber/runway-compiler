...
void Generator::emitVariableDeclarationStatement(
  ...
  AssignmentExpression *assignment_expr =
      ((AssignmentExpression *) var_decl_stmt->expression_to_assign);
  Expression *expression = assignment_expr->expression_to_assign;

  std::string identifier = assignment_expr->identifier;

  llvm::Value *llvm_value = expression->emit(this);

  if (llvm_value == 0) {
    std::cerr << "VariableDeclarationStatement: emitted value is empty"
        << std::endl;
    return;
  }

  llvm::Constant *llvm_const = (llvm::Constant *) llvm_value;

  llvm::GlobalVariable *gv = new llvm::GlobalVariable(
      *_module, llvm_const->getType(), true, llvm::GlobalValue::ExternalLinkage,
      llvm_const, "testval");
  _values[identifier] = gv;
}
...
llvm::Value *Generator::emitPrimaryExpression(PrimaryExpression *expr) {
  ...
  if (expr_type == ExpressionType::BOOL) {
    ...
    return llvm::ConstantInt::getIntegerValue(
        ...
        llvm::APInt(32, value));
  } else if (expr_type == ExpressionType::IDENTIFIER) {
    return _values[expr->string_value];
  }
  return nullptr;
}
